...
/*enum bidstates {NONE,
		...
		MONO_CO0,MONO_NH1,MONO_CO2,MONO_NH3}; */	//combinations of peptide groups
enum pepOne {CO0, NH1, CO2, NH3};	//peptide groups
...
*/
void print_state(int time, int state[14], bool diid, double** rama){
	for(int i=0; i<14; i++){
		cout<<state[i]<<"  ";
	}
	cout<<diid<<"   "<<rama[time][0]<<"  "<<rama[time][1]<<endl;
}
...
bidstates classifyBidInos(int dipepHbonds[]){
...
int main(int argc, char* argv[]){
	...
	while(readGroFile(gro,dipep,inositols)){
		...
		for(int ins=0; ins<inositols.size(); ins++){
			...
			switch(numPepHbonds){
				...
				case 1: //monodentate
					...
					break;
				default:
					inos_mode=classifyBidInos(dipepHbonds);
					inositols[ins]->setBindMode(inos_mode);
					inositols[ins]->setHbondedPep();
					break;
			}
		...
		cout<<endl;
#endif
		print_state(time,snapshot,diid,ramaHistogram);
#ifdef DEBUG
		cout<<endl;
#endif
		inositols.resize(0);
