...
trap 'clean; exit $?' TERM INT KILL SIGINT EXIT
...
function concatenate_and_fit {
    ...
    echo "Protein Protein_GLCA" | trjconv -f /dev/shm/grace/$xtc_file -s $TPR -fit rot+trans -o ${xtc_file}_fit.xtc -n protein_glca.ndx
...
vmd -dispdev text -e /home/grace/labwork/abeta_analysis/volmap/volmap_glca.tcl -args ${xtc_file}_fit.xtc $GRO > /dev/shm/grace/${xtc_file}_fit.log 2>&1 &
wait
