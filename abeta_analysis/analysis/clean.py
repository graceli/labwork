...
def process_hbonds(h5file, analysis_type):
...
def main():
    if len(sys.argv) < 2:
        print "usage: clean.py <analysis>"
        sys.exit(0)

    analysis = sys.argv[1] 
    h5file = tables.openFile("analysis.h5", 'a')
    # reading hbonds for all inositol molecules
    if analysis == "hbonds_inositol":
        process_hbonds(h5file, "inositol")
    
    if analysis == "hbonds_residue":
        process_hbonds(h5file, "residue")
