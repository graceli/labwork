...
class NonpolarContactAnalysis(Analysis):
    def _command(self, chain_first, chain_last, insgrp_first, insgrp_last, xtc_filename, system_index, output_dir, testing=False):
        ...
        shell_command = string.Template("seq $chain_first $chain_last | parallel -j 16 \"echo {} $index_groups | g_inositol_residue_nonpolar_v2 -f $data_dir/$xtc_name -s ${iso}_${ratio}_nosol.tpr -n ab_${ratio}_${iso}_nonpolar_revised.ndx -per_residue_contacts $output_dir/${sys_index}_chain{}_residue_np_contact.dat -per_inositol_contacts $output_dir/${sys_index}_chain{}_inositol_np_contact.dat -per_residue_table $output_dir/${sys_index}_chain{}_table.dat -num_inositols $num_ligands $testing\"").substitute(chain_first=chain_first, chain_last=chain_last, insgrp_first=insgrp_first, insgrp_last=insgrp_last, data_dir=self.DATA_BASE_DIR, xtc_name=xtc_filename, iso=self.ISO, ratio=self.RATIO, output_dir=output_dir, sys_index=system_index, num_ligands=self.RATIO, index_groups=insgroups, testing='-b 0 -e 100')
...
if __name__ == "__main__":
    ...
    temp = '/dev/shm/grace'
    analysis.analyze_contact_by_inositol(Analysis.ISO, Analysis.RATIO, Analysis.RATIO, temp, testing=True)
