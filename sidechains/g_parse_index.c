...
int main(int argc,char *argv[]) {
	...
	"Modified to work with Gromacs 4.0 (compiled with Gromacs 4.0.5 libraries)"
	};
...
//             },
	...
	matrix     box;
	t_pbc 	   pbc;
	int        ePBC=-1;  //added for read_tps_conf
	...
	//read_tps_conf usage from gmx_mindist 4.0.5
	//  bTop = read_tps_conf(tpsfnm,title,top,&ePBC,&x,NULL,box,FALSE); -- the TRUE means 
	// read masses -- assumed to not matter in the rest of the code
	read_tps_conf(ftp2fn_null(efTPS,NFILE,fnm), title, &top, &ePBC, &xtop, NULL, box, TRUE);
	sfree(xtop);
	...
	do {
               ...
               //cout << "\n############  FRAME # " << t << " #############" << endl;
                cout<<"\n"<< t <<" ";
#endif
	       //set pbc each step b/c of pressure coupling
