...
# *.class file fields
# added 1 column for the stereochemistry of the OH groups; appears before NONE
...
sub gather{
	...
	while($line=<FILE>){
		...
		if($sum1_12 > 0){
			...
			if($fields[1]){
				print BID1 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[2]){
				print BID2 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[3]){
				print BID3 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[4]){
				print BID4 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[5]){
				print BID5 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[6]){
				print BID6 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[7]){
				print BID7 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[8]){
				print BID8 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[9]){
				print BID9 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[10]){
				print BID10 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[11]){
				print BID11 "$fields[15] $fields[16] $fields[17]\n";
			}
			if($fields[12]){
				print BID12 "$fields[15] $fields[16] $fields[17]\n";
			}
		}
	...
	return($avg_bid,$avg_mono,$avg_none,$avg_diid,$avg_num_multi_bound,$avg_num_inos_bound,$kd);
}
