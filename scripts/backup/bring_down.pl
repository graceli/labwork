...
my $use_ccb_rsync="--rsh=ssh --rsync-path=/tools/rsync/2.6.9/bin/rsync";

system("rsync $use_ccb_rsync -av --progress $use_dry_run $use_remove ${cluster}:$dir_from/DATA/*/*.xtc .");
#print("rsync $use_ccb_rsync -av --progress $use_dry_run $use_remove ${cluster}:$dir_from/DATA/*/*.xtc .");

if($get_success eq "y"){
	system("rsync $use_ccb_rsync -av --progress $use_dry_run $use_remove ${cluster}:$dir_from/*_success/*desorted.xtc .");
}
